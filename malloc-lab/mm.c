...
/* GET: Read raw word value from memory location */
#define GET(p)            (*(unsigned int *)(p))
/* PUT: Write value while preserving reallocation tag bit */
#define PUT(p, val)       (*(unsigned int *)(p) = (val) | GET_TAG(p))
/* PUT_NOTAG: Write value while clearing reallocation tag bit */
#define PUT_NOTAG(p, val) (*(unsigned int *)(p) = (val))

/* Store predecessor or successor pointer for free blocks */
#define SET_PTR(p, ptr) (*(unsigned int *)(p) = (unsigned int)(ptr))

/* Adjust the reallocation tag - bit 1 (0x2) prevents coalescing/allocating */
#define SET_TAG(p)   (*(unsigned int *)(p) = GET(p) | 0x2)
#define UNSET_TAG(p) (*(unsigned int *)(p) = GET(p) & ~0x2)

/* Read the size and allocation bit from address p */
#define GET_SIZE(p)  (GET(p) & ~0x7)  /* Clear lower 3 bits */
#define GET_ALLOC(p) (GET(p) & 0x1)   /* Extract alloc bit (bit 0) */
#define GET_TAG(p)   (GET(p) & 0x2)   /* Extract realloc tag (bit 1) */
...
#define PRED_PTR(ptr) ((char *)(ptr))           /* First 4 bytes of free block */
#define SUCC_PTR(ptr) ((char *)(ptr) + WSIZE)   /* Next 4 bytes of free block */
...
#define CHECK_FREE    1 /* Check free operations (I forgot to do this at first) */
...
/*
 * Function prototypes (I didn't know what a prototype did before this lol)
 */
...
int mm_init(void)
{
  ...
  /* Initialize 16-byte initial heap structure:
   * [padding(4)][prologue hdr(8)][prologue ftr(8)][epilogue hdr(0)] (idk I found this in an article)*/
  ...
  /* Extend the empty heap with first chunk */
...
void *mm_malloc(size_t size)
{
  ...
  */
  ...
  /* Small requests get minimum 16B block, others get 8-byte aligned */
  if (size <= DSIZE) {
    asize = 2 * DSIZE;
  } else {
  ...
  /* Start with exact size class, fall back to larger classes */
  size = asize;
  while (list < LISTS) {
    /* Check this list (last list catches all remaining sizes) */
    if ((list == LISTS - 1) || ((size <= 1) && (free_lists[list] != NULL))) {
      ptr = free_lists[list];
      /* Skip blocks too small or with reallocation tag set */
      while ((ptr != NULL)
        && ((asize > GET_SIZE(HEAD(ptr))) || (GET_TAG(HEAD(ptr)))))
      {
        ptr = PRED(ptr);  /* Traverse list backward (address order) */
      }
      if (ptr != NULL)
        break;
    }
    
    size >>= 1;  /* Try next larger size class */
    list++;
  }
...
void mm_free(void *ptr)
{
  ...
  /* Allows future coalescing with this newly freed block */
...
void *mm_realloc(void *ptr, size_t size)
{
  ...
  /* Reserve buffer space for future reallocations */
  new_size += BUFFER;
  
  /* Calculate block buffer */
  /* How much extra space remains after requested size + buffer */
  ...
  if (block_buffer < 0) {
    ...
    if (!GET_ALLOC(HEAD(NEXT(ptr))) || !GET_SIZE(HEAD(NEXT(ptr)))) {
      /* Can expand into next free block */
      remainder = GET_SIZE(HEAD(ptr)) + GET_SIZE(HEAD(NEXT(ptr))) - new_size;
      if (remainder < 0) {
        /* Still need more space: extend heap */
      ...
      /* Remove next block from free list, expand current block */
      delete_node(NEXT(ptr));
      
      /* Do not split block - keep remainder as buffer */
      PUT_NOTAG(HEAD(ptr), PACK(new_size + remainder, 1)); /* Block header */
      PUT_NOTAG(FOOT(ptr), PACK(new_size + remainder, 1)); /* Block footer */
    } else {
      /* Next block not free: malloc/copy/free strategy */
  ...
  /* Warns adjacent free block not to coalesce (needs buffer space) */
...
static void *extend_heap(size_t size)
{
  ...
  /* Initialize new free block at end of heap */
...
static void insert_node(void *ptr, size_t size) {
  ...
  /* Select segregated list by right-shifting size to find class */
  ...
  /* Select location on list to insert pointer while keeping list
     organized by memory address in ascending order (search backward) */
  ...
  /* Set predecessor and successor pointers (4 cases handled) */
  if (search_ptr != NULL) {
    if (insert_ptr != NULL) {
...
static void delete_node(void *ptr) {
  ...
  /* Unlink from doubly-linked list (4 cases: interior/head/tail/alone) */
  if (PRED(ptr) != NULL) {
    if (SUCC(ptr) != NULL) {
      /* Interior node: connect pred and succ */
      SET_PTR(SUCC_PTR(PRED(ptr)), SUCC(ptr));
      SET_PTR(PRED_PTR(SUCC(ptr)), PRED(ptr));
    } else {
      /* Tail node: remove from pred's succ, update list head */
      SET_PTR(SUCC_PTR(PRED(ptr)), NULL);
      free_lists[list] = PRED(ptr);
    }
  } else {
    if (SUCC(ptr) != NULL) {
      /* Head node: update succ's pred, list head becomes succ */
...
static void *coalesce(void *ptr)
{
  ...
  /* Reallocation tag blocks are "reserved" and shouldn't be merged */
  ...
  if (prev_alloc && !next_alloc) {
    /* Case 1: Only merge with next */
    delete_node(NEXT(ptr));
    size += GET_SIZE(HEAD(NEXT(ptr)));
    PUT(HEAD(ptr), PACK(size, 0));
    PUT(FOOT(ptr), PACK(size, 0));
  } else if (!prev_alloc && next_alloc) {
    /* Case 2: Only merge with prev */
    ...
    /* Case 3: Merge with both prev and next */
    delete_node(PREV(ptr));
    delete_node(NEXT(ptr));
...
static void place(void *ptr, size_t asize)
{
  ...
  if (remainder >= MINSIZE) {
    /* Split block into allocated portion + remainder free block */
    ...
    /* Do not split block - use entire free block */
